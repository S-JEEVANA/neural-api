...
const int analogInPin = A0;  // Analog input pin that patient sensor is attached to
const int analogInPin1 = A1;  // Analog input pin that blower sensor is attached to

float PatientsensorValue = 0;        // value read from the patient sensor
//float PatientsensorValue1 = 0;
int count=0;

void patientSensor()
{
  PatientsensorValue = analogRead(analogInPin);
  PatientsensorValue = PatientsensorValue * (5.0 / 1023);;
  //PatientsensorValue = (int)(PatientsensorValue *100 +0.5);
  //PatientsensorValue = (float)(PatientsensorValue/100);
  
  Serial.print("\t Patient sensor = ");
  Serial.println(PatientsensorValue);
}
...
void u8g_CPAP_On(void) {
  ...
  u8g.firstPage();
    do {
      ...
      u8g.print("Apnea Detected");
    } while ( u8g.nextPage() );
}
void CPAP_Off_draw(void) {
  u8g_prepare();
  u8g_CPAP_Off();
}

void CPAP_On_draw(void) {
  u8g_prepare();
  u8g_CPAP_On();
}
void setup(void) {
...
void loop(void) {
  ...
  if((PatientsensorValue==2.47) ||(PatientsensorValue==2.47) ||(PatientsensorValue==2.48) ||(PatientsensorValue==2.49) || (PatientsensorValue==2.50)|| (PatientsensorValue==2.51)|| (PatientsensorValue==2.52)|| (PatientsensorValue==2.53)|| (PatientsensorValue==2.54))
  {
    count++;
    if(count>=2000)
    {
